...
pub(crate) async fn get_login(
    ...
    let (allow_registrations, bitbucket_sso_enabled, github_sso_enabled, gitlab_sso_enabled): (
        bool,
        bool,
        bool,
        bool,
...
pub(crate) async fn post_login(
    ...
    if option.is_none() {
        debug!(
            "Received login request for non-existent user: {}",
            &username
        );
    ...
    if user.password == "sso-login" {
        debug!(
            "Received login request for an {} (id {}) despite being registered with SSO",
    ...
    if !crypto::check_password(&user, password)? {
        debug!(
            "Received login request with wrong password for {} (id {})",
            &user.username, &user.id
        );
    ...
    if user.disabled || !primary_email.is_allowed_login() {
        debug!(
            "Received login request for disabled user {} (id {})",
            &user.username, &user.id
        );

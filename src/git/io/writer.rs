...
impl GitWriter {
    ...
    pub(crate) async fn write_binary_sideband(&mut self, band: Band, binary: &[u8]) -> Result<&mut GitWriter> {
        ...
        self.inner.enable_binary_mode();
        self.inner.write(with_band.as_slice()).await.with_context(|| {
            format!("Unable to write binary to sideband {} in Git writer: {:?}", band, binary)
        })?;
    ...
    pub(crate) async fn write_raw(&mut self, binary: &[u8]) -> Result<&mut GitWriter> {
        self.inner.inner_mut().write(binary).await.with_context(|| format!("Unable to write raw data to Git writer: {:?}", binary))?;
        Ok(self)
    }
    ...
    pub(crate) async fn flush_sideband(&mut self, band: Band) -> Result<&mut GitWriter> {
        ...
        self.inner.enable_binary_mode();
        self.inner.write(with_band.as_slice()).await.with_context(|| {
            format!("Unable to write flush to sideband {} in Git writer", band)
        })?;
    ...
    pub(crate) async fn append(&mut self, other: GitWriter) -> Result<&mut GitWriter> {
        ...
        self.write_raw(serialized.to_vec().as_slice()).await.context("Unable to write other Git writer to Git writer")?;

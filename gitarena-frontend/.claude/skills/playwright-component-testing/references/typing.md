...
export function storyTypes(options: { prefix: string; src: string; outFile: string }): Plugin {
    const generate = () => {
        ...
        if (fs.existsSync(options.outFile) && fs.readFileSync(options.outFile, "utf8") === content) return;
    ...
    return {
        ...
        configureServer(server) {
            const onFile = (file: string) => {
                if (storyFile.test(file)) generate();
            };
...
function listStoryFiles(dir: string): string[] {
    ...
    for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
        ...
        if (entry.isDirectory()) result.push(...listStoryFiles(file));
        else if (storyFile.test(entry.name)) result.push(file);
    }
    return result.sort();
}

...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Personal security audit log", body = Vec<EventResponse>),
        (status = 401, description = "Authentication required"),
    ),
    security(("cookieAuth" = [])),
    tag = "user"
)]
#[route("/api/users/me/audit-log", method = "GET", err = "json")]
pub(crate) async fn get_personal_audit_log(web_user: WebUser, query: web::Query<EventListParams>, db_pool: web::Data<Pool>) -> Result<impl Responder> {

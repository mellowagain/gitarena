...
pub(crate) async fn dashboard(web_user: WebUser, db_pool: web::Data<Pool>) -> Result<impl Responder> {
    ...
    let (users_count,): (i64,) = sqlx::query_as("select count(*) from users").fetch_one(&mut *transaction).await?;
    ...
    let (repos_count,): (i64,) = sqlx::query_as("select count(*) from repositories").fetch_one(&mut *transaction).await?;
    ...
    if let Some(latest_repo) = latest_repo_option {
        ...
        let (latest_repo_username_option,): (String,) = sqlx::query_as(
    ...
    let (postgres_version,): (String,) = sqlx::query_as("show server_version").fetch_one(&mut *transaction).await?;

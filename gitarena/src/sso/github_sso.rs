...
impl SSOProvider for GitHubSSO {
    ...
    fn validate_scopes(&self, scopes_option: Option<&Vec<Scope>>) -> bool {
        let granted_scopes = match scopes_option {
            Some(granted_scopes) => granted_scopes.iter().flat_map(|scope| scope.split(',')).collect::<Vec<_>>(),
    ...
    async fn create_user(&self, token: &str, db_pool: &Pool) -> Result<User> {
        ...
        for github_email in emails.iter().skip_while(|e| !e.verified) {
            ...
            sqlx::query(
                "insert into emails (id, owner, email, \"primary\", commit, notification, public, verified_at) values ($1, $2, $3, $4, $4, $4, $5, current_timestamp)",
            )
            .bind(Uuid::now_v7())
            .bind(user.id)
            .bind(email)
